...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id_supplier) {
        // Update Supplier
        $stmt = $conn->prepare("UPDATE supplier SET nama_supplier = ?, deskripsi = ? WHERE id = ?");
        $stmt->bind_param("ssi", $nama_supplier, $deskripsi, $id_supplier);
        if ($stmt->execute()) {
        ...
        $stmt = $conn->prepare("INSERT INTO supplier (nama_supplier, deskripsi) VALUES (?, ?)");
        $stmt->bind_param("ss", $nama_supplier, $deskripsi);
        if ($stmt->execute()) {
...
$supplier_data = [];
$result = $conn->query("SELECT id, nama_supplier, deskripsi FROM supplier ORDER BY id ASC");
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $id_edit = $_GET['id'];
    $stmt = $conn->prepare("SELECT id, nama_supplier, deskripsi FROM supplier WHERE id = ?");
...
<div class="form-section">
    ...
    <form action="supplier.php" method="POST">
        ...
        <div class="form-group">
            ...
            <textarea id="deskripsi" name="deskripsi" rows="4"><?php echo $edit_supplier ? htmlspecialchars($edit_supplier['deskripsi']) : ''; ?></textarea>
        </div>
            ...
            <tbody>
                <?php foreach ($supplier_data as $supplier): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($supplier['deskripsi']); ?></td>
                        <td>
